...
const TrainerCard = ({ item }) => {
    const { name, category, image, socialIcons, availableWeek, availableDay, experienceYears, _id } = item
    return (
            ...
            <div className="card-body space-y-2">
                <div className="flex justify-between items-center">
                    ...
                    <div className='flex text-2xl mt-5'>
                        <Link to={socialIcons?.facebook}><FaFacebook className='text-blue-500 bg-white rounded-full'></FaFacebook></Link>
                        <FaTwitter className='text-blue-500 ml-5'></FaTwitter>
                    </div>
                </div>
                <div className="text-gray-400">
                    <p className="text-lg">Experience: {experienceYears}</p>
                    <h3>Available:{availableWeek}, {availableDay}</h3>
                </div>
                <h2 className="card-title text-xl">{category}</h2>

                <Link to={`availableTrainer/${_id}`}><button className="btn px-4 w-full bg-amber-500 text-white mt-10">Know More</button></Link>
